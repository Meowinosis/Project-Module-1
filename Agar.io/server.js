...
const initialNumCells = 30; // Adjust as needed
const speed = 1;
...
function generateInitialCells(numCells) {
    ...
    for (let i = 0; i < numCells; i++) {
        const cellX = Math.random() * 800; // Adjust the range based on your canvas size
        const cellY = Math.random() * 600; // Adjust the range based on your canvas size
        const cellSize = Math.floor(Math.random() * (maxCellSize - minCellSize)) + minCellSize;
...
io.on("connection", (socket) => {
    ...
    socket.on("joinGame", (playerData) => {
        ...
        const player = new Player(socket.id, startingX, startingY, startingPlayerSize, getRandomColor(), name);
        player.x = startingX;
        player.y = startingY;
        ...
        socket.on("playerMove", (mousePosition) => {
            ...
            if (player) {
                const now = Date.now();
                if (now - player.lastPositionUpdateTime >= positionUpdateInterval) {
                    player.lastPositionUpdateTime = now;
    ...
    socket.movementInterval = setInterval(() => {
    ...
    setInterval(() => {
        ...
        io.emit("leaderBoards", sortedPlayers.map((player, index) => ({
            rank: index + 1,
            name: player.name
        })));
    }, 1000 / 10);
